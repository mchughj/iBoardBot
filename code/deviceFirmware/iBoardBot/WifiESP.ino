...
uint8_t ESPwaitFor(const char *stopstr, int timeout_secs)
{
  ...
  while (!found) {
    if (((millis() - timer_init) / 1000) > timeout_secs) { // Timeout?
      Serial.println(F("!Timeout in ESPwaitFor!"));
      return 0;  // timeout
    }
    ESPreadChar(c);
...
uint8_t ESPwaitFor2(const char *stopstr, const char *stopstr2, int timeout_secs)
{
  ...
  while (found == 0) {
    if (((millis() - timer_init) / 1000) > timeout_secs) { // Timeout?
      Serial.println(F("!Timeout in ESPwaitFor2!"));
      return 0;  // timeout
    }
    ESPreadChar(c);
...
uint8_t ESPgetMac(char *MAC)
{
  ...
  Serial.println(F("!Timeout getting MAC!"));
  Serial1.flush();
  return -1;  // timeout
}
...
uint8_t ESPsendHTTP(char *url)
{
  ...
  Serial.print(F("ESPsendHTTP - getting free memory; freeRam: "));
  Serial.println(freeRam());
  Serial.print(F("ESPsendHTTP - going to ; url: "));
  Serial.println(url);
  ...
  if (ESPwaitFor("OK", 5))
  {
    ...
    ESPwaitFor(">", 3);
    Serial1.println(cmd_get);
    Serial.print( F("ESPsendHTTP - established connection; sending: "));
    ...
    digitalWrite(13,LOW);
    Serial.println(F("ESPsendHTTP - Connection error"));
